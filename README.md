...
JSON を返す API からデータを取得し、カードを 1 枚ずつめくって学習し、結果(OK / not OK)を同じ API へ POST します。
...
登録した URL に対して `GET` し、以下の形式の **配列** を受け取ります。
...
`public/sample.json` を同梱しています。API 登録画面で URL に `./sample.json` を指定すると GET 部分の動作を確認できます(静的ファイルのため POST は失敗し、画面下に通知が出ますが学習は続行できます)。
