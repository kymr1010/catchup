...
- `contents` … 回答を編集した場合のみ送信します。未編集の場合は含めません
- `Content-Type: text/plain;charset=utf-8` で JSON 文字列を送ります
...
`authorizationToken` は任意です。設定した場合、GET と POST の両方で `Authorization: Bearer <token>` として送信されます。既存データに `Bearer ` 付きで保存されている場合は、その値をそのまま送信します。
