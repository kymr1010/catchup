...
- **表示順 (`order`)** — API ごとに localStorage に保存されます
  ...
  - `memorize` … 暗記(エビングハウスの忘却曲線に沿った復習順)
    - **更新日時(`date`)が新しい(=直近学習した)ものほど優先**します(直近の復習が最重要)
    - **正答数(`ok_count`)が多いものほど優先度を下げ**、出る頻度を落とします(間隔とは独立した重み付け)
    - **学習から 3 分以内**のものは、同じカードの連続表示を避けるため今回の並びから除外します
    - **正答数 0 かつ `date` 無し(未学習)**のものは、重み付けを無視してランダムに混ぜ込みます(さもないと永遠に出なくなるため)
- **学習フロー**
